...
class PromptCache:
    ...
    def get_prompt_hash(self, prompt_ids: list[int]) -> str:
        ...
        Returns:
            Hex string hash
        """
        # Encode as a delimited string: token IDs exceed 255 (vocab is large),
        # so bytes(prompt_ids) would raise ValueError. The delimiter prevents
        # collisions between e.g. [1, 23] and [12, 3].
        tokens_bytes = ",".join(map(str, prompt_ids)).encode("utf-8")
