...
class KVCacheManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized KVCacheManager with {num_pages} pages, "
            f"{page_size} tokens per page"
        )
    ...
    def read_kv(self, seq_id: str, length: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if seq_id not in self.used_pages:
            return (
                torch.zeros((0, self.num_heads, self.head_dim), dtype=self.dtype, device=self.device),
                torch.zeros((0, self.num_heads, self.head_dim), dtype=self.dtype, device=self.device),
            )
    ...
    def _evict_lru_pages(self, num_pages_to_free: int):
        ...
        evicted = 0
        for seq_id in sorted(self.used_pages.keys(), key=lambda s: len(self.used_pages[s]), reverse=True):
    ...
    def get_cache_stats(self) -> dict:
        ...
        return {
            ...
            "memory_used_mb": (total_cached_pages * self.page_size * 2 * self.num_heads * self.head_dim * 2) / (1024 * 1024),
        }
