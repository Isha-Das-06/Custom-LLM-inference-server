...
def main():
    ...
    parser.add_argument("--completion-length", type=int, default=256, help="Completion length")
    ...
    logger.info(f"Generating {args.num_requests} prompts with length {args.prompt_length}")

    # Generate dummy prompts
    prompts = [" ".join(["hello"] * (args.prompt_length // 6)) for _ in range(args.num_requests)]
    ...
    with torch.no_grad():
        for i in range(0, args.num_requests, args.batch_size):
            ...
            total_tokens += outputs.shape[0] * outputs.shape[1] * args.completion_length // 100
