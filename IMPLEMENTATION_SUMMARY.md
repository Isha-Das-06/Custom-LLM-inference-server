...
### 9. **Inference Engine** (`src/core/inference_engine.py`)
- `InferenceEngine` class: Main orchestration loop
  - Continuously schedules batches
...
**Key Methods:**
- `run()`: Main async loop (10ms polling)
- `_process_batch()`: Process one batch iteration
- `_update_kv_cache()`: Store K,V tensors
- `_complete_request()`: Finalize and record metrics
- `get_metrics()`: Export system statistics

**Lines of Code:** ~200

### 10. **GPU Utilities** (`src/utils/gpu_utils.py`)
- `GPUMemoryMonitor` class: GPU memory tracking
  - Memory allocated/reserved tracking
  - Device information
