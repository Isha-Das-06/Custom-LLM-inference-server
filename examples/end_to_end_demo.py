...
async def health_and_metrics():
    ...
    async with httpx.AsyncClient() as client:
        ...
        print(
            f"  GPU memory: {metrics['gpu_memory_used_gb']:.2f}GB / {metrics['gpu_memory_total_gb']:.2f}GB"
        )
        print(f"  Cache hit rate: {metrics['cache_hit_rate']:.1%}")
        print(
            f"  KV-cache pages: {metrics['kv_cache_pages_used']}/{metrics['kv_cache_pages_total']}"
        )
